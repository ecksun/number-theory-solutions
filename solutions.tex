...
\noindent{One} has to be careful when implementing the comparison operator between two fractions. Note that $a/b > c/d$ ($b,d \neq 0$) is the same as $a*d > c*b$, which gives better precision.
...
\noindent{In} the next step we take a H-prime number from the previous list we created. We
