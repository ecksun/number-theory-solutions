...
\subsection{Time complexity}
The time complexity for finding the LCM of two numbers $a,b$ is $\mathcal{O}(\log{max(a,b)})$. Total time complexity is $\mathcal{O}(k \cdot log{p_i})$, where $p_i$ is the periodicity and $k$ the number of wheels.
...
\subsection{Time complexity}
The time complexity of the solution is $\mathcal{O}(P)$, where P is the path length.
% TODO
%fraction left(0,1)
%fraction right(1,0)
