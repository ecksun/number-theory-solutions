...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \STATE prime[] \COMMENT{A boolean list with size N, all set to true}
        \FOR{$i = 2 \to N$}
            \FOR{$j = 2 \to N/i$}
...
The problem statement asks us to find the jackpot periodicityi $P$ of an entire machine, given the jackpot periodicities $p_{1},p_{2},...,p_{n}$ for the individual wheels in the machine. The answer is the smallest integer P such that $p_{i}$ divides P, for all $i \in {1,2,...,n}$.
