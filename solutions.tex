...
\noindent{Use} the Sieve of Eratosthenes (Algorithm \ref{lst:primesieve}) to generate all
...
\noindent{Before} returning the number of factors in a number, we save it in a table, to
...
\\\\
\noindent{This} integer is found by computing the least common multiple of integers
$p_{1},p_{2},...,p_{n}$. LCM can be computed using an implementation of GCD
(algorithm \ref{lst:LCM}), in the following way:
...
\\\\
\noindent{We} need an efficient way to store the rests we had and where they appeared. The
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \STATE $r = p \mod q$
        \STATE $seen[r] = 0$ \COMMENT{First rest seen at index 0}
        \STATE $i = 0$
        \WHILE {$r \neq 0$}
            \STATE $r = r \cdot 10$
...
\noindent{If} the decimal expansion is finite (e.g. we get rest zero in the division), the
...
$p'$ and $q'$. 
\\\\
\noindent{We} call the current node $C$. We start our path in the root node with $C =
...
$\frac{0}{1}$ and $\frac{1}{0}$. Note that we change the $L$ and $R$ pointer when switching nodes.
...
\noindent{In} the algorithm we use a simple
structure, fraction, which stores a fraction as its numerator and denominator.
...
\noindent{One} has to be careful when implementing the comparison operator between two fractions. 
Note that $a/b > c/d$ ($b,d \neq 0$) is the same as $a \cdot d > c \cdot b$, which gives better precision.
...
\noindent{The} naive solution is to simply use trial division to factor each number from
...
\noindent{Since} we need to calculate the Mobius function for all values in the range we
...
\noindent{The} next step is to calculate the number of factors in each number. For each
number we can simply use trial division to remove one factor and recursively
call the Mobius function for that value. As
performance will be an issue when doing this we can simply cache
...
\noindent{The} Mertens function has to be calculated in $O(N)$ time, simply iterate through all
...
This problem asks us to count the number of H-semi-primes within a certain range. We do this in three steps.

\noindent{In} the first step we want to create a list of all H-prime numbers in the
sequence created by $4n + 1$ for any positive $n$. An H-prime is the product of
...
find all non-prime H-numbers and mark those in the list.
...
\noindent{In} the next step we take an H-prime number from the previous list we created. We
then take another H-prime number which is the same or larger and calculate the
product of those. The product of those are a semi-prime H-number. This way, we generate 
all possible semi-prime H-numbers.
...
\noindent{What} remains to be done is to count the semi-prime H-numbers. After doing this, one can get the answer from the counting array.
